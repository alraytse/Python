...
def collect_device(
    ...
    try:
        ...
        for block in blocks:
            ...
            if not gateway:
                notes.append("Gateway not found in Infoblox output")
...
def main():
    ...
    parser.add_argument(
        "--match",
        default="Azure Local",
        help='text required in the network name or comment; default: "Azure Local"',
    )
    ...
    parser.add_argument(
        ...
        help=f"directory for raw Infoblox output; default: {DEFAULT_RAW_DIR}",
    )
    ...
    parser.add_argument(
        ...
        help=f"command read timeout in seconds; default: {DEFAULT_TIMEOUT}",
    )
    ...
    if not csv_file:
        print("No Infoblox CSV file was provided.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"Unable to load Infoblox CSV: {exc}", file=sys.stderr)
        sys.exit(1)
