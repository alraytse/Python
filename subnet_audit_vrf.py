...
"""VRF-aware subnet audit for Cisco IOS/IOS-XE, NX-OS, and Arista EOS."""
...
def build_route_command(subnet, vrf, device_type):
...
def build_arp_command(vrf, device_type):
    if vrf:
        return f"show ip arp vrf {vrf}"
    return "show ip arp"


def build_ping_command(target, vrf, device_type):
...
def process_device(host, username, password, device_type, vrf, subnets):
    ...
    try:
        logging.info("%s: connecting with %s, VRF=%s", host, device_type, vrf or "GLOBAL")
        conn = ConnectHandler(
        ...
        arp_output = send_command(conn, host, build_arp_command(vrf, device_type))

        for subnet in subnets:
            route_command = build_route_command(subnet, vrf, device_type)
            try:
            ...
            if collection_status == "ERROR":
                ...
                try:
                    ping_command = build_ping_command(ping_target, vrf, device_type)
            ...
            logging.info(
                "%s: subnet=%s vrf=%s route=%s reachability=%s",
                host,
                subnet,
                vrf or "GLOBAL",
                route["route_type"],
...
def main():
    logging.info("Subnet VRF audit started")
    ...
    logging.info("Subnet VRF audit completed: %d records, %d failed devices", len(results), len(FAILED_DEVICES))
