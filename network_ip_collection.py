...
SWITCH_NAT_CSV_FILE = "switch_nat_report.csv"
...
# ---------------------------------------------------------------------------
# Best-effort switch NAT collection
# ---------------------------------------------------------------------------


def parse_nat_output(output, device, platform, command):
    rows = []
    for raw_line in output.splitlines():
        line = raw_line.strip()
        if not line or line.startswith(("--", "Protocol", "Inside", "Outside", "Total")):
            continue
        ips = extract_ips(line)
        lower = line.lower()
        record_type = "PAT" if any(term in lower for term in ("pat", "overload", "port")) else "NAT"
        rows.append({
            "Device": device,
            "Platform": platform,
            "Record_Type": record_type,
            "Command": command,
            "Inside_Local": ips[0] if len(ips) > 0 else "",
            "Inside_Global": ips[1] if len(ips) > 1 else "",
            "Outside_Local": ips[2] if len(ips) > 2 else "",
            "Outside_Global": ips[3] if len(ips) > 3 else "",
            "Details": line,
        })
    return rows


def collect_nxos_nat(connection, device):
    commands = (
    ...
    for command in commands:
        output = safe_send_command(connection, command, read_timeout=30)
        if output:
            rows.extend(parse_nat_output(output, device, "cisco_nxos", command))
    return rows
...
def firewall_profile(platform):
    profiles = {
        "cisco_asa": {
            ...
            "nat": ["show nat detail", "show xlate", "show running-config nat", "show running-config global"],
            "tunnel": [
        ...
        "generic": {
            ...
            "nat": ["show nat", "show nat translations", "show xlate"],
...
def parse_firewall_interfaces(output, device, platform, command):
    ...
    for raw_line in output.splitlines():
        ...
        # Cisco-style brief output has rows such as:
        ...
        if (
            ...
            # Cisco-style brief output: interface, IP, OK/method, status, protocol.
...
def collect_firewall(device, username, password, platform):
    ...
    try:
        ...
        for command in profile["interface"]:
            ...
            if output:
                rows.extend(parse_firewall_interfaces(output, device_name, platform, command))

        for command in profile["nat"]:
            output = safe_send_command(connection, command, read_timeout=60)
            if output:
                rows.extend(parse_firewall_text_rows(output, device_name, platform, command, "NAT/PAT"))

        for command in profile["tunnel"]:
            output = safe_send_command(connection, command, read_timeout=60)
            if output:
                rows.extend(parse_firewall_text_rows(output, device_name, platform, command, "TUNNEL"))
...
def build_parser():
    ...
    parser.add_argument("--switch-hosts", help="Comma-delimited NX-OS switch hostnames/IPs.")
    parser.add_argument("--firewall-hosts", help="Comma-delimited firewall hostnames/IPs.")
    parser.add_argument(
        "--firewall-platform",
    ...
    parser.add_argument(
        "--switch-nat-csv-file",
        default=SWITCH_NAT_CSV_FILE,
        help=f"NX-OS NAT CSV output. Default: {SWITCH_NAT_CSV_FILE}",
    )
    parser.add_argument(
        "--firewall-csv-file",
...
def get_credentials(label, default_username=None, default_password=None):
    if default_username is None:
        ...
        entered = input(f"{label} username [{default_username}]: ").strip()
    ...
    if switch_hosts:
        ...
        for host in switch_hosts:
            ...
            try:
                ...
                all_interface_results.extend(collect_nxos_interfaces(connection, hostname))
                all_switch_nat_results.extend(collect_nxos_nat(connection, hostname))
            except Exception as error:
        ...
        write_rows(all_switch_nat_results, args.switch_nat_csv_file, SWITCH_NAT_FIELDS)
        print(f"STP CSV saved to: {args.csv_file}")
        print(f"NX-OS interface/IP CSV saved to: {args.interface_csv_file}")
        print(f"NX-OS NAT CSV saved to: {args.switch_nat_csv_file}")
